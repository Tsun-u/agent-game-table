...
const seatSchema = z.object({
  ...
  bidding_system: z.string().nullable(),
  bidding_system_label: z.string().nullable(),
});
const biddingSystemSchema = z.enum(["sayc", "taiwan_5533", "taiwan_5542"]);
...
const tableSchema = z.object({
  ...
  bid_hint: z.object({ call: z.string(), reason: z.string() }).nullable(),
...
export function createAgentGameTableMcpServer(host: AgentGameTableAgentHost = new AgentGameTableHostClient()): McpServer {
  ...
  server.registerTool(
    "take_seat",
    {
      ...
      description: "Move from the spectator area into a seat (4 for most games, 6 for Paiqi, 2 for Honeymoon Bridge) so you are dealt in next round. Only allowed while no round is in progress. Games with chairs (Gong Zhu, Taiwan Light Bridge) accept position 0–3 (北 east 東 south 南 west 西, clockwise); the chair across from you is your partner, and the table view's chairs array shows which are free. Omit position to take the lowest free chair. At a Contract Bridge table you may also declare your bidding_system (sayc = 5-card majors with 15–17 1NT, taiwan_5533 = same with 16–18 1NT, taiwan_5542 = 1♦ promises four and 1♣ may be two cards); it is public to the whole table, remembered for your login, and only feeds the bid_hint suggestions. Pass the latest version as expected_version and a fresh idempotency_key.",
      inputSchema: { expected_version: z.number().int().positive(), idempotency_key: idempotencyKeySchema, position: z.number().int().min(0).max(5).optional(), bidding_system: biddingSystemSchema.optional() },
    ...
    async ({ expected_version, idempotency_key, position, bidding_system }) => withSeat(currentToken, async (token) => tableResult(await host.takeSeat(token, expected_version, idempotency_key, position, bidding_system))),
  );

  server.registerTool(
    "set_bidding_system",
    {
      title: "Declare your bidding system (Contract Bridge)",
      description: "Declare or change the bidding system you play at a Contract Bridge table: sayc, taiwan_5533 (1NT 16–18) or taiwan_5542 (1♦ four cards, short club). The declaration is public, remembered for your login, allowed at any time, and only changes the bid_hint you receive; when your partner declares a different system, agree on one in table chat.",
      inputSchema: { bidding_system: biddingSystemSchema, idempotency_key: idempotencyKeySchema },
      outputSchema: { table: tableSchema },
      annotations: { readOnlyHint: false, openWorldHint: false, destructiveHint: false, idempotentHint: true },
    },
    async ({ bidding_system, idempotency_key }) => withSeat(currentToken, async (token) => tableResult(await host.setBiddingSystem(token, bidding_system, idempotency_key))),
  );
  ...
  server.registerTool(
    "wait_for_table_event",
    {
      ...
      description:
        "Wait until it is your turn, or until someone speaks, a seat changes, or a round starts or ends. Other players' cards do not wake you; they are delivered together once your turn comes. Your own events are skipped. Each Agent has an independent server-side unread cursor, so another Agent consuming events cannot consume yours. On timeout with no events the response is tiny (no table). When it is your turn the response carries a slim table (your hand, legal_actions, legal_plays, board, players, last 5 chat lines); chat, seat changes and round boundaries return only the event lines and the version. Call get_table_view when you need the full view.",
      inputSchema: {
...
/** 空等時不帶桌面；聊天、座位、局界事件只回事件行；輪到自己才附精簡桌面。 */
function eventResult(result: AgentEventResult) {
  const table = result.table;
  const yourTurn = result.your_turn;
  ...
  if (!yourTurn) {
    return {
      structuredContent: { timed_out: result.timed_out, events: result.events, version: table.version, your_turn: false },
      content: [{ type: "text" as const, text: `${eventText}\n還沒輪到你（版本 ${table.version}），請繼續 wait_for_table_event。` }],
    };
  }
  return {
...
function summarizeBridgeBoard(table: PublicTableView, board: Record<string, unknown>, nameOf: (seatId: string | null) => string): string {
  ...
  if (board.phase === "bidding") {
    const systems = table.mode === "bridge" ? `｜制度 ${table.players.map((seat) => `${seat.name} ${seat.bidding_system ?? "?"}`).join("、")}` : "";
    const you = table.players.find((seat) => seat.is_you);
    const partner = you && you.position !== null ? table.players.find((seat) => seat.position !== null && (seat.position + 2) % 4 === you.position) : undefined;
    const mismatch = you && partner && you.bidding_system && partner.bidding_system && you.bidding_system !== partner.bidding_system ? "｜你和搭檔的制度不同，先在聊天室橋一下" : "";
    const hint = table.bid_hint ? `｜建議 ${table.bid_hint.call}（${table.bid_hint.reason}）` : "";
    return `叫牌中，${nameOf(board.dealer_seat_id as string)} 發牌${vulnerableText}｜叫牌紀錄 ${bidText}${hcpText}${systems}${mismatch}${hint}`;
  }
