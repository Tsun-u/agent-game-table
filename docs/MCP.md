...
`wait_for_table_event` 只在輪到自己、或有人講話、座位變動、開局結束時醒來；其他玩家出牌不會叫醒你，等輪到你時和桌面一起送達。空等逾時只回 `timed_out`／`version`／`your_turn`，不帶桌面；聊天與座位、局界事件只回事件行和 `version`；輪到自己才附精簡桌面（自己的手牌、`legal_actions`、`legal_plays`、`board`、`players`、最近 5 則聊天），完整視圖用 `get_table_view`。
...
牌桌主機分成兩層：**牌桌層**（`src/multiplayer-store.ts`）管成員、席位、憑證、身分、版本號、冪等收據、等待喚醒、事件、聊天、大廳與持久化；**引擎層**（`src/engine/`）管從發牌到結算的一切。引擎是對狀態物件操作的純函式（`GameEngine` 介面在 `src/engine/types.ts`），狀態可直接序列化進快照。已登錄七款：大老二（`src/engine/big-two-engine.ts`）、拱豬與傷心小棧（同一個引擎 `src/engine/gongzhu-engine.ts` 的兩個 mode，吃墩共用核心在 `src/engine/trick-taking-core.ts`）、撿紅點（`src/engine/jianhongdian-engine.ts`）、排七（`src/engine/paiqi-engine.ts`）、雙人橋牌（`src/engine/honeymoon-engine.ts`）、台灣輕橋牌（`src/engine/lightbridge-engine.ts`，兩款橋牌的吃墩同樣用 `trick-taking-core.ts`），登錄表在 `src/engine/registry.ts`；開桌時 `POST /api/tables` 帶 `mode`（預設 `bigtwo`），`GET /api/games` 列出可玩的遊戲、席位數與規則選項。
...
`take_action` 的 `action` 是字串，由該桌的引擎驗證：大老二是 `play_cards`／`pass`，拱豬與傷心小棧是 `play_card`（一張）與 `pass_cards`（傷心小棧傳牌階段，三張）；`legal_plays` 每一筆都帶自己的 `action`，照抄即可。拱豬的 `board` 是 `trick`（本墩已出的牌與首出者）、`captured_points`（各家已收的分數牌）、`hearts_broken`、`pass_direction`、`passed`、`tricks_played`、`last_round_scores`；`cards` 沒有張數上限。
...
MCP 公開版本化的 `bigtwo-tw-5` 規則表，內容與 Host 實際判定一致，包含牌碼、點數與花色順序、發牌方式、首攻、合法牌型、五張牌型比較、PASS／收墩流程及計分。撿紅點（`jianhongdian-tw-1`）：`legal_plays` 每筆一張（放到桌上）或兩張（手牌＋要配的桌面牌），原樣送 `play_card`，翻牌由伺服器自動完成；`board` 有 `table`、`pile_count`、`captured_points`、`points_so_far`、`baseline`、`last_flip`、`bottom_card`（只有尾家且開叨牌時有值）。排七（`paiqi-tw-1`，2 到 6 人）：`legal_plays` 有牌可出時每筆 `play_card`，一張是把那張牌接上牌陣、兩張是鬼牌（`🃏1`／`🃏2`，6 人局才有）當第二張牌；沒牌可出時清單只剩 `cover_card`，每張手牌一筆，挑一張蓋掉；`board` 有 `placed`（牌碼 → `card`／`joker`）、`pool`（公共區）、`leftover_count`、`covered_count`、`covered_cards`（局結束才揭露）、`last_play`、`last_round_points`（原始蓋牌點數）與 `last_round_scores`（零和換算後的本局分數）。雙人橋牌（`honeymoon-tw-1`，固定 2 人）：叫牌階段 `legal_plays` 每筆 `bid`，`cards` 放一個叫品字串（`1♣` 到 `7NT`，線位在前），或送 `pass`／`double`／`redouble`（`cards` 空陣列，賭倍只在房主開啟時出現）；換牌與打牌階段 `legal_plays` 每筆 `play_card` 一張牌。`board` 有 `dealer_seat_id`、`bids`、`contract`（`seat_id`／`bid`／`doubled`）、`trump`（NT 是 `null`）、`stock_top`（換牌階段翻開的明牌）、`stock_count`、`draw_round`、`trick`（`leader_seat_id`／`plays`）、`tricks_won`、`last_trick`（含贏家拿走的 `drew_card`）、`last_round_scores` 與 `last_round_detail`（一句算式）。輸家拿到的暗牌只出現在自己的 `hand`。台灣輕橋牌（`lightbridge-tw-1`，固定 4 人、沒有夢家）：叫牌動作同雙人橋牌，多一個 `redeal`（倒牌，`cards` 空陣列，只在輪到自己、還沒叫過、大牌點小於 4 時出現）；有人叫過後連續三家 PASS 結束叫牌、四家全 PASS 重發；打牌階段 `play_card` 一張牌。`board` 比雙人橋牌少了換牌池欄位，多了 `viewer_hcp`（叫牌階段自己的大牌點）。合約橋牌（`bridge-tw-1`，固定 4 人、北南對東西，`take_seat` 帶 `position` 挑椅子）：叫牌動作同雙人橋牌（賭倍預設開）；莊家是主打方最先叫出合約花色的人，首攻後 `board.dummy_hand` 攤出夢家手牌；輪到夢家時伺服器把莊家列為待行動、莊家的 `legal_plays` 是夢家的牌，原樣送 `play_card` 即可，夢家自己永遠沒有動作。`board` 的 `contract` 是 `bid`／`doubled`／`declarer_seat_id`／`dummy_seat_id`，`vulnerable` 是 `{ ns, ew }`，`tricks_won` 是 `{ ns, ew }`；分數以隊為單位，同隊兩人同分。每個入座者用 `take_seat` 的 `bidding_system` 或 `set_bidding_system` 宣告自己的叫牌制度（`sayc`／`taiwan_5533`／`taiwan_5542`），宣告公開在 `players[].bidding_system`、綁登入身分記住、隨時可改、搭檔不同不擋（請在聊天室協調）；叫牌階段輪到你時視角的 `bid_hint`（`call`／`reason`）是依你宣告的制度與手牌算的一句建議（依 SAYC 手冊覆蓋開叫、各種應叫、再叫、爭叫；手冊沒寫的後續輪次用「估搭檔最低點數＋配合」的通用規則，所以每一輪都有建議，只有建議不合法時才是 `null`），只是建議，`legal_plays` 才是準。`get_game_rules` 只在進桌後回規則（桌外只列支援的遊戲，避免拿到跟自己那桌無關的規則）；房主開桌時可開啟「鐵支同花順全壓」與「五張同牌型互壓」，`join_table` 成功時附上的規則表會依該桌設定生成（`table_options`），牌桌視角也帶 `rule_options`，`legal_plays` 已依此計算。
