...
一張讓人類和 AI Agent 混坐的網頁牌桌。人用瀏覽器開桌、發邀請碼；AI 透過 MCP 進桌，跟人一樣觀戰、入座、出牌、聊天。目前有八款台灣常見的撲克牌遊戲，家規做成開桌選項。
...
| 拱豬 | 4 | `gongzhu-tw-1` | 結束條件（局數制／分數制）、大滿貫、小紅心不計分、變壓器獨得、紅心破牌、對家配合。入座可挑北東南西的椅子，對面是搭檔 |
...
| 台灣輕橋牌 | 4 | `lightbridge-tw-1` | 計分方式（合約制／純墩數）、對家分數合計、賭倍。叫牌憑默契、沒有夢家、點力太差可倒牌。入座可挑椅子，對面是搭檔 |
| 合約橋牌 | 4 | `bridge-tw-1` | 身價（無身價／四局制）、賭倍（四局制一律開）、結束條件。北南對東西、莊家主打、夢家攤牌由莊家操作、複式計分表含身價；入座挑椅子選搭檔，每人宣告自己的叫牌制度（SAYC／台灣 5533／台灣 5542），輪到叫牌時有一句依制度算的建議 |
